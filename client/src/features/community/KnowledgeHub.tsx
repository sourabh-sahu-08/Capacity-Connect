...
export const KnowledgeHub = () => {
  return (
    <div className="p-8 text-white max-w-7xl mx-auto flex gap-8">
      <div className="flex-1 space-y-8">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold">Knowledge Hub</h1>
            <p className="text-zinc-400 mt-2">Share insights, ask questions, and learn from peers.</p>
          </div>
        ...
        <div className="flex gap-4">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full bg-zinc-900 border border-zinc-800 rounded-lg py-3 pl-10 pr-4 text-white placeholder-zinc-500 focus:outline-none focus:border-indigo-500 transition-colors"
            />
          </div>
        </div>
        ...
        <div className="flex gap-4 border-b border-zinc-800 pb-px">
          {['Recent', 'Most Voted', 'Unanswered', 'My Posts'].map((tab, i) => (
            <button key={tab} className={`pb-3 px-2 text-sm font-medium border-b-2 transition-colors ${i === 0 ? 'border-indigo-500 text-indigo-400' : 'border-transparent text-zinc-400 hover:text-zinc-200'}`}>
              {tab}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-4">
          {POSTS.map(post => (
            <div key={post.id} className="bg-zinc-900 border border-zinc-800 p-6 rounded-xl hover:border-zinc-700 transition-colors">
              <div className="flex gap-4">
                <div className="flex flex-col items-center gap-2">
                  <button className="text-zinc-400 hover:text-indigo-400 p-1"><ThumbsUp size={20} /></button>
                ...
                <div className="flex-1">
                  ...
                  <h3 className="text-xl font-bold mb-2 hover:text-indigo-400 cursor-pointer">{post.title}</h3>
                  <p className="text-zinc-400 text-sm line-clamp-2 mb-4">{post.content}</p>
                  
                  <div className="flex items-center justify-between">
                    <div className="flex gap-2">
                      {post.tags.map(tag => (
                        <span key={tag} className="flex items-center gap-1 text-xs font-medium text-indigo-300 bg-indigo-500/10 px-2 py-1 rounded">
                          <Tag size={12} /> {tag}
                        </span>
                      ))}
                    </div>
                    <div className="flex items-center gap-4 text-sm text-zinc-400">
                      {post.hasBestAnswer && (
                        <span className="flex items-center gap-1 text-emerald-400 bg-emerald-500/10 px-2 py-1 rounded font-medium">
      ...
      <div className="w-80 hidden lg:block space-y-6">
        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-xl">
          <h3 className="font-bold mb-4 flex items-center gap-2"><Award className="text-amber-500" /> Top Contributors</h3>
          <div className="space-y-4">
            {[
              ...
              <div key={user.name} className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-zinc-800 flex items-center justify-center font-bold text-sm">{user.avatar}</div>
                <div className="flex-1">
                  <div className="text-sm font-medium">{user.name}</div>
                  <div className="text-xs text-zinc-500">{user.score} points</div>
                </div>
                <div className="text-amber-500 font-bold text-sm">#{i+1}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-xl">
          ...
          <div className="flex flex-wrap gap-2">
            {['React', 'Node.js', 'System Design', 'AWS', 'TypeScript', 'CSS'].map(tag => (
              <span key={tag} className="text-xs font-medium text-zinc-300 bg-zinc-800 px-2 py-1 rounded hover:bg-zinc-700 cursor-pointer">
                {tag}
              </span>
            ))}
          </div>
