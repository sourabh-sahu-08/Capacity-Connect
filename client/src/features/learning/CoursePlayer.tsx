    ...
    <div className="h-[calc(100vh-6rem)] -m-8 flex flex-col md:flex-row bg-slate-50 text-slate-900 overflow-hidden">
            ...
            <div key={mod} className="space-y-1">
              ...
              {i === 1 && (
                <div className="pl-4 space-y-1 border-l border-slate-200 ml-2">
                  <div className="flex gap-2 items-center text-sm py-2 text-indigo-600 bg-indigo-50 px-2 rounded">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white">
        <div className="flex-1 relative bg-white flex items-center justify-center">
          ...
          <button className="relative z-10 w-20 h-20 bg-indigo-600/90 hover:bg-indigo-600 rounded-full flex items-center justify-center text-white  transition-transform hover:scale-105">
      ...
      <div className="w-full md:w-96 border-l border-slate-200 bg-white flex flex-col">
        <div className="flex border-b border-slate-200">
          <button 
            className={`flex-1 py-4 text-sm font-medium flex items-center justify-center gap-2 border-b-2 ${activeTab === 'copilot' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-slate-600 hover:text-slate-800'}`}
            onClick={() => setActiveTab('copilot')}
          >
            <Sparkles size={16} /> AI Copilot
          </button>
          <button 
            className={`flex-1 py-4 text-sm font-medium flex items-center justify-center gap-2 border-b-2 ${activeTab === 'notes' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-slate-600 hover:text-slate-800'}`}
          ...
          <>
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.map((msg, i) => (
                ...
                >
                  <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-slate-100 text-slate-800 rounded-bl-none border border-slate-300'}`}>
                    {msg.content}
                  </div>
                </motion.div>
              ))}
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-200 space-y-3">
              ...
              <div className="flex gap-2">
                <input 
                  ...
                  className="flex-1 bg-white border border-slate-300 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-indigo-500"
                />
                <button 
                  onClick={handleSend}
                  disabled={!input.trim()}
